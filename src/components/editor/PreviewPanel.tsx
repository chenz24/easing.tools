...
import { UIScenePanel } from "./UIScenePreview";
...
export function PreviewPanel({
  ...
  const [previewTab, setPreviewTab] = useState<"shapes" | "scenes" | "compare">("shapes");
  const {
    previews,
    addPreview,
    updateShape,
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <Tabs
        value={previewTab}
        onValueChange={(v) => setPreviewTab(v as "shapes" | "scenes" | "compare")}
      ...
      >
        <div className="border-b border-border px-4 py-2">
          <TabsList className="h-8 w-full bg-muted">
            ...
            <TabsTrigger value="compare" className="flex-1 text-xs data-[state=active]:bg-surface">
              Compare
            </TabsTrigger>
          </TabsList>
        </div>
      ...
      {/* Add Preview Button - only show for shapes tab */}
      {previewTab === "shapes" && (
